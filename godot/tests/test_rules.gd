...
func _initialize():
 ...
 check(p.alive and p.xp==0 and p.light==0 and p.spirit==0 and p.depth==1,"fresh fox starts clean")
 ...
 check(saved.alive and saved.xp==p.xp and saved.spirit==37 and saved.upgrades.power==1,"profile round-trip preserves living progression and absorbed levels")
 check(Rules.stats(saved).damage>Rules.stats(Rules.fresh(first[0],12)).damage,"absorbed enemy levels increase living fox strength")
