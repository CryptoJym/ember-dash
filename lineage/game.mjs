...
function heirCard(h, index) { const p = powerOf(h), echo = ECHOES.find(x => x.id === h.echo), art = {ember:'vulpax',tide:'nivalis',gale:'sylra',void:'umbra',sun:'lumen',bloom:'verdara'}[p.id]; return `<button class="heir-card ${game.heir.index === index ? 'selected' : ''}" data-heir="${index}" style="--power:${p.color}" aria-pressed="${game.heir.index === index}"><span class="card-top"><span>${p.mark} ${p.name}</span><span class="selection">${game.heir.index === index ? '✓' : '○'}</span></span><span class="heir-art"><img src="lineage/assets/foxes/${art}.png" alt="${p.name} fox"></span><span class="heir-name">${h.name}</span><span class="heir-skill">${p.skill}</span><span class="heir-detail">${p.detail}</span><span class="echo">${echo.name} <small>${echo.description}</small></span></button>`; }
function showHearth() {
    setMode('hearth');
    chrome();
    game.heirs = heirsFor(profile);
    game.heir = game.heirs[game.heir?.generation === profile.generation ? game.heir.index : 0];
    panel.className = `hearth-panel${!profile.lineage.length && profile.wallet === 0 ? ' first-hearth' : ''}`;
...
function startGame(seed) { audio.unlock(); game.seed = seed || entropy(); game.runId = entropy(); game.boons = {}; game.stats = getStats(profile, game.heir, game.boons); game.room = makeRoom(game.seed, 1); game.player = newPlayer(game.room, game.stats); game.camera = 0; game.t = 0; game.light = 0; game.pickups = 0; game.xp = 0; game.nextBoon = 24; game.boonPending = false; game.death = null; game.dashSerial = 0; clearEffects(); if (!beginExpedition(profile, game.heir, game.runId)) {
    ...
    return;
} save(); setMode('playing'); updateHud(); toast(`${game.heir.name} of ${powerOf().name}. ${matchMedia('(pointer:coarse)').matches ? 'Move with the left pad. Hold JUMP for height; tap again in the air.' : 'A/D to move. Hold Space for height; tap again in the air.'}`); }
...
function update(dt) {
    ...
    const lookAhead = clamp(p.vx * .42, -135, 185);
    const target = clamp(p.x + lookAhead - visibleWidth * .36, 0, Math.max(0, r.width - visibleWidth));
    game.camera += (target - game.camera) * (1 - Math.exp(-11 * dt));
    if (doInteract)
        interact();
