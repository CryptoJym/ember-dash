...
async function beginContext(options={}){
 ...
 async function click(prefix){await page.waitForTimeout(230);let s=await state();const b=s.menuButtons.find(b=>b.text.startsWith(prefix));assert.ok(b,'button '+prefix);const v=page.viewportSize();const scaleX=v.width/(s.viewport?.[0]||v.width),scaleY=v.height/(s.viewport?.[1]||v.height);const x=(b.rect[0]+b.rect[2]/2)*scaleX,y=(b.rect[1]+b.rect[3]/2)*scaleY;assert.ok(y<v.height&&x<v.width&&y>0,'visible button '+prefix);await page.mouse.click(x,y);await page.waitForTimeout(230);}
...
try{
 ...
 let s=await state();organic=s;writeFileSync(out+'/traversal-state.json',JSON.stringify(s,null,2));await page.screenshot({path:out+'/traversal-finish.png'});ok('real keyboard traversal completes a seeded chamber',s.mode==='camp');ok('defeated enemies raise the living fox above level one',s.level>=2&&s.xp>0);ok('collecting and defeating enemies earns spendable light',s.light>=18);
 await page.screenshot({path:out+'/sanctuary-earned.png'});
 const wallet=s.light;await click('Inner flame');let saved=JSON.parse(await page.evaluate(k=>localStorage.getItem(k),KEY));ok('earned currency buys a real stat upgrade',saved.upgrades.power===1&&saved.light===wallet-18);
 if((await state()).menuButtons.some(b=>b.text.startsWith('Awaken a new gift'))){await click('Awaken a new gift');await click('Feather soul');saved=JSON.parse(await page.evaluate(k=>localStorage.getItem(k),KEY));ok('earned level grants a chosen extra air jump',saved.talents.wings===1&&saved.tokens===0);}
 await click('Save and rest');await page.reload();await d.ready();await click('Resume your living');await page.waitForFunction(()=>window.emberStatus.mode==='playing');s=await state();ok('earned XP and level survive a browser reload',s.xp===saved.xp&&s.name===saved.name);
 ...
 await page.screenshot({path:out+'/death-reset.png'});await click('Start a new life');await click('Begin as');await page.waitForFunction(()=>window.emberStatus.mode==='playing');s=await state();ok('new character starts at level one with zero XP and light',s.level===1&&s.xp===0&&s.light===0);saved=JSON.parse(await page.evaluate(k=>localStorage.getItem(k),KEY));ok('dead character upgrades and talents do not carry over',saved.upgrades.power===0&&Object.keys(saved.talents).length===0);
 ...
 const tab=await d.context.newPage();await tab.goto(new URL('index.html',url).href);await tab.evaluate(k=>localStorage.setItem(k,JSON.stringify({version:1,alive:false})),KEY);await tab.close();await page.bringToFront();await page.waitForTimeout(2400);
 if((await state()).mode==='paused'){await click('Continue');await page.waitForTimeout(2500);}
